 ...
 * Prompt          : imageArray and imageUrlArray can only use one. If you use two, 
 *                   view will select imageArray to use. Only one image can not auto carouse(本地图片数组与图片地址数组只能同时使
 *                   用一个, 如果同时使用两个，那么当前轮播控件默认使用 本地图片数组 来做轮播。只有一张图片的时候不会自动轮播。)
 *
 * @funtions:
 * isAutoCarouseling : return the image carousel state, stop or playing. (是否正在轮播，返回
 *                     bool值，YES是正在轮播，NO是停止轮播)
 * startAutoCarousel : start to carousel image, default is start state. Image cout at least two. (开始轮播，默认是开始播
 *                     放状态。图片个数最少是2张。)
 * stopAutoCarousel  : stop to carousel image, default is can not stop. (停止轮播，默认是不会停
 *                     止轮播图片。)
 **/
...
- (void)CarouselDelegateIndex:(NSInteger)index;
