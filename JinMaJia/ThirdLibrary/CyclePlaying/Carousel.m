...
{
    NSIndexPath *zeroIndexPath = [NSIndexPath indexPathForItem:1 inSection:0];
    [self.cycleCollectionView scrollToItemAtIndexPath:zeroIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
    if (!newSuperview)
    {
        [_timer invalidate];
        _timer = nil;
...
- (void)startAutoCarousel
{
    ...
    {
        if ([self imageCount] >= 3) //图片个数必须最少2张，才能自动轮播
        {
            [self startTimer];
        }
    }
...
- (void)setImageArray:(NSArray *)imageArray
{
    ...
    [arrayM addObject:imageArray.lastObject];
    [arrayM addObjectsFromArray:imageArray];
    [arrayM addObject:imageArray.firstObject];
    _imageArray = arrayM;
...
{
    NSMutableArray *arrayM = [NSMutableArray array];
    [arrayM addObject:imageUrlArray.lastObject];
    [arrayM addObjectsFromArray:imageUrlArray];
    [arrayM addObject:imageUrlArray.firstObject];
    _imageUrlArray = arrayM;
    
    if (self.pageControl)
    {
        self.pageControl.numberOfPages = _imageUrlArray.count - 2;
    }
...
//        });
        CGFloat offSetX = _cycleCollectionView.contentOffset.x/SCREENW;
        NSIndexPath *zeroIndexPath = [NSIndexPath indexPathForItem:offSetX+1 inSection:0];
        [_cycleCollectionView scrollToItemAtIndexPath:zeroIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:YES];
    }
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_imageArray) {
        ...
        array = _imageUrlArray;
    }
    if([_delegate respondsToSelector:@selector(CarouselDelegateIndex:)]){
        NSInteger num = indexPath.item - 1;
        if (num == array.count - 2) {
            num = 0;
        }else if (num == -1) {
            num = array.count - 3;
        }
        [_delegate CarouselDelegateIndex:num];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    NSInteger pageNum = offSet / SCREENW;
    ...
    if (pageNum == array.count - 1) {
        NSIndexPath *zeroIndexPath = [NSIndexPath indexPathForItem:1 inSection:0];
        [_cycleCollectionView scrollToItemAtIndexPath:zeroIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
        pageNum = 0;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    NSInteger pageNum = offSet / SCREENW;
    if (pageNum == 0) {
        NSIndexPath *zeroIndexPath = [NSIndexPath indexPathForItem:array.count - 2 inSection:0];
        [_cycleCollectionView scrollToItemAtIndexPath:zeroIndexPath atScrollPosition:UICollectionViewScrollPositionLeft animated:NO];
    }
}
//- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
